...
<div id = "poll_{POLL_ID}">
	<form action="{POLL_FORM_URL}" method="post" id="poll_form_{POLL_ID}" class="ajax post-poll_{POLL_ID};index.php;e=polls&mode=ajax&poll_theme=index">
		...
		<ul style="list-style-type: none; list-style-image: none; margin: 0px;">
			...
			<!-- END: POLLTABLE -->
			<li><button type="submit" class="btn" title="{PHP.L.polls_Vote}">{PHP.L.polls_Vote}</button></li>
		</ul>
	</form>
</div>
<!-- END: POLL_VIEW -->
...
<table style="width: 100%;">
	...
	<tr style="width: 100%;">
		<td>{POLL_OPTIONS}</td>
		<td style="text-align: right;">{POLL_PER}% ({POLL_COUNT})</td>
	</tr>
	<tr>
		<td colspan="2">
			<div class="bar_back">
...
<p style="text-align: right;">{PHP.L.Votes}: {POLL_VOTERS}</p>
<!-- END: POLL_VIEW_VOTED -->
...
<!-- END: ERROR -->
<div style="text-align: right;"><a href="{IPOLLS_ALL}">{PHP.L.polls_viewarchives}</a></div>
<!-- END: INDEXPOLLS -->